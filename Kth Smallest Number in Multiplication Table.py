...
class Solution(object):
    def findKthNumber(self, m, n, k):
        ...
        while low < high:
            mid = (low + high) // 2
            if self.num_values(mid, m, n) >= k:
                high = mid
            else:
                low = mid + 1
        return low

    # Number of values in the i-th row that are <= 'val' = min( val // i, num_cols)
    def num_values(self, val, m, n):
        count = 0
...
if __name__ == '__main__':
    sol = Solution()
    print(sol.findKthNumber(m=3, n=3, k=5))