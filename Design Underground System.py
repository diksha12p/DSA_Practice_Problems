...
// return 11.00000. There were two travels from "Leyton" to "Waterloo", a customer with id=45 from time=3 to time=15 and
// a customer with id=27 from time=10 to time=20. So the average time is ( (15-3) + (20-10) ) / 2 = 11.00000
...
class UndergroundSystem:
    ...
    def checkOut(self, id: int, stationName: str, t: int) -> None:
        start_st, start_t = self.passenger[id]
        self.log[(start_st, stationName)].append(t - start_t)

    def getAverageTime(self, startStation: str, endStation: str) -> float:
        return sum(self.log[(startStation, endStation)]) / len(self.log[(startStation, endStation)])
